document.addEventListener('DOMContentLoaded', () => {
    ...
    const segResultImg = document.getElementById('seg-result-img');
    ...
    const segEvacMsg = document.getElementById('seg-evac-msg');
    ...
    analyzeBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (response.ok && data.prediction && !data.error) {
                // ═══════════════════════════════════════════════════════════
                // CLASSIFICATION RESULTS (Repo A — original logic)
                // ═══════════════════════════════════════════════════════════
                ...
                // ═══════════════════════════════════════════════════════════
                // SEGMENTATION RESULTS (Repo B — new panel)
                // ═══════════════════════════════════════════════════════════
                if (data.segmentation_available && data.result_image_path) {
                    // Show segmented image
                    segResultImg.src = data.result_image_path;

                    // Build severity bars
                    severityBars.innerHTML = '';
                    if (data.stats && data.stats.classifications) {
                        data.stats.classifications.forEach(c => {
                            const row = document.createElement('div');
                            row.className = 'sev-row';
                            row.innerHTML = `
                                <div class="sev-label-row">
                                    <span class="sev-label" style="color:${c.bar_color}">${c.label}</span>
                                    <span class="sev-note">${c.note}</span>
                                    <span class="sev-pct">${c.pct}%</span>
                                </div>
                                <div class="bar-track">
                                    <div class="bar-fill" style="width:${c.pct}%; background:${c.bar_color};"></div>
                                </div>
                            `;
                            severityBars.appendChild(row);
                        });
                    }

                    // Danger badge
                    if (data.stats.danger_level) {
                        dangerBadge.textContent = `${data.stats.danger_icon} ${data.stats.danger_level}`;
                        dangerBadge.style.color = data.stats.danger_color;
                        dangerBadge.style.borderColor = data.stats.danger_color;
                        dangerBadge.style.background = data.stats.danger_color + '1a';
                    }

                    // Survival chance
                    if (data.stats.survival !== undefined) {
                        survivalValue.textContent = `${data.stats.survival}%`;
                        if (data.stats.survival >= 70) survivalValue.style.color = '#2ecc71';
                        else if (data.stats.survival >= 40) survivalValue.style.color = '#f1c40f';
                        else survivalValue.style.color = '#e74c3c';
                    }

                    // Evacuation message
                    if (data.stats.evac_msg) {
                        segEvacMsg.textContent = data.stats.evac_msg;
                    }

                    // Show the segmentation card
